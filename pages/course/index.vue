...
export default {
  async asyncData(page) { // 此处传递当前页面的上下文对象，以便获取查询条件
    ...
    // 获取课程分类嵌套列表
    const subjectNestedListResponse = await subjectApi.multiLevelSubjectList()
    ...
    for (let i = 0; i < subjectNestedList.length; i++) {
      ...
      if (subjectNestedList[i].id === searchObj.subjectParentId) {
        // 则获取二级分类列表
        subSubjectList = subjectNestedList[i].childs
      }
    }
    // 课程列表
    const courseListResponse = await courseApi.getCourseListByCondition(searchObj)
